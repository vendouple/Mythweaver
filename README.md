...
## Roleplay model guidance

- Narration targets must support OpenAI-compatible tool calling. The Weaver resolves declared attempts, reads dice results, then finishes with `narrate_turn`; model-generated dice numbers are not authoritative.
- Prompts prioritize player agency, distinct NPC voices, short dialogue beats, and flexible story plans. Quiet conversation does not require a roll or a new threat.
- Character images, including NPCs and portrait backfill, request square `1024x1024` output. Existing artwork is preserved and cropped in the UI. Providers without size support receive explicit square-composition guidance in the image prompt.
- Image jobs run asynchronously. A `queued` response is not an image URL, and slow pending portraits are not regenerated by backfill.
- Run `npm test` for offline regression checks with mocked model/image responses, plus `npm run typecheck` and `npm run lint`. These checks do not spend API credits or modify saved campaigns. Actual storytelling quality still needs playtesting with your chosen model.
