 ...
 * (see src/lib/client/sfx.ts for cue names). Long environmental beds live as
 * public/music/AMBIENCE/<category>.mp3 and are exposed as separate loop layers.
 */
export async function GET() {
  try {
    ...
    const ambienceFiles = await listAudioFiles(ambienceRoot);
    for (const file of ambienceFiles) {
      const category = file.replace(AUDIO_EXT, "").toLowerCase();
      (ambience[category] ||= []).push(`/music/AMBIENCE/${file}`);
    }
