...
A small, persistent HTTP service that owns a single
...
Design constraints:
    * Binds to the local network by default so phones and the host can see its
      address; the Next.js app itself still connects through localhost.
...
def local_ip() -> str:
    """Best-effort LAN address for the startup message (never blocks startup)."""
    try:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    lan_ip = local_ip()
    LOG.info("TTS server listening on http://%s:%d", args.host, args.port)
    LOG.info("Host URL: http://127.0.0.1:%d | LAN URL: http://%s:%d", args.port, lan_ip, args.port)
    LOG.info("Voice directory: %s", VOICE_DIR)
    try:
